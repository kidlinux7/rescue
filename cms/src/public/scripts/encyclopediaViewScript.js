...
$('#articleModal').on('show.bs.modal', event => {
  ...
  if (articleId === 0) {
    $('.modal-title').text('Ongeza Makala Mpya')
    $('#col0TableModal').val('')
    $('#col1TableModal').val('')
    $('#col2TableModal').val('')
    $('#col3TableModal').val('')
    ...
    $('#itemID').text(0)
    $('#countdown2').text(70 + ' tarakimu zilizobakia')
  ...
  $('#countdown2').text(70- articleInfo.article_heading.length + ' tarakimu zilizobakia')
...
$('#categoryModal').on('show.bs.modal', event => {
  ...
  $('#errorCat3').hide()
  if (categoryId === 0) {
    $('.modal-title').text('Weka Kipengele kipya')
...
$('#subcategoryModal').on('show.bs.modal', event => {
  ...
  $('#errorSubcat2').hide()
  if (subcategoryId === 0) {
    $('.modal-title').text('Ongeza Kipengele Kidogo kipya')
...
$('.deleteArticle').on('click', event => {
  ...
  var articleId = button.data('value') // Extract info from data-* attributes
  var result = confirm('Una uhakika? Hii itafuta kipengele hiki kabisa')
  if (result) {
    $.ajax({
      url: '/articles/' + articleId,
      type: 'DELETE',
...
function makeUpdateCountdown({ countdownElement, tableElement, maxLength }) {
  function updateCountdown() {
    ...
    countdownElement.text(remaining + ' tarakimu zilizobakia.')
  }
